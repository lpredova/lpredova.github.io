...
class Pico_Share
{
    ...
    public $templates = array(
        'mail me' => 'mailto:lovro.predovan@gmail.com',
        'read more' => 'http://lovropredovan.com/blog',
    ...
    public function before_render(&$twig_vars, &$twig, &$template)
    {
        ...
        foreach ($this->config['services'] as $key => $value) {
            if (is_bool($value) && $value) {
                $activeServices[$key] = '<a class="' . $this->config['class_prefix'] . $key . '" target="_blank" href="' .
                    preg_replace(array('/__TITLE__/', '/__URL__/', '/__EXCERPT__/'),
                        array($pageTitle, $pageURL, $pageExcerpt), $this->templates[$key]) .
                    '">' . $key . '</a>';
            };
        };
        switch ($this->config['output']) {
            ...
            default:
                ...
                $twig_vars['social_share'] = '<ul id="share"><li>' . implode('</li><li>',
                        array_values($activeServices)) . '</li></ul>';
                break;
        };
    }
}